...
from pb_loader import ToCamelName
...
%     if len(code_index.fields) == 1:
                var key${code_index.camelname} = (${code_index.get_cs_key_type_list()})itemInfo.${code_index.get_cs_key_params()};
%     else:
                var key${code_index.camelname} = new ValueTuple<${code_index.get_cs_key_type_list()}>(
...
%         if len(code_index.fields) == 1:
                if (ConfigSetUtils.IsDefault(key${code_index.camelname})) break;
%         else:
                if (ConfigSetUtils.IsAnyDefault(key${code_index.camelname})) break;
%         endif
%       endif
...
%         if len(code_index.fields) == 1:
                if (ConfigSetUtils.IsDefault(key${code_index.camelname})) break;
%         else:
                if (ConfigSetUtils.IsAllDefault(key${code_index.camelname})) break;
%         endif
%       endif
%       if code_index.is_list():
